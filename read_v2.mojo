...
from itertools.itertools import drop_while, take_while, _DropWhileIterator, _TakeWhileIterator
...
from memory.span import _SpanIter as SpanItr
import os
...
fn parse_and_update_kv_pairs[
    o: Origin, I: Iterator, //, separator: Byte, end_char: Byte
](mut data: I, mut base: TomlType[o]) -> _DropWhileIterator[
        InnerIteratorType=_DropWhileIterator[
            InnerIteratorType=type_of(data),
            any[T=Byte, Space, NewLine]
        ]
    ]:
    """This function ends at end_char always."""
    ref dta = rebind[SpanItr[Byte, o]](data)
    var without_spaces = drop_while[any[Space, NewLine]](dta)

    find_kv_and_update_base[end_char=end_char](without_spaces,base)    

    return drop_while[any[end_char, neg=True]](without_spaces)
    var stop_at_char = take_while[any[end_char]](without_spaces)

    
        
    for c in take_while[](data):
...
# @always_inline
# fn skip[*chars: Byte](mut data: SpanItr[Byte]):
#     for c in data:
...
# @always_inline
# fn stop_at[*chars: Byte](data: SpanItr[Byte]):
#     for c in data:
...
fn any[T: Movable, //, *chars: T, neg: Bool = False](value: T) -> Bool:
    comptime assert conforms_to(T, Comparable & ImplicitlyCopyable)
    ref v= trait_downcast[Comparable & ImplicitlyCopyable](value)
    @parameter
    for ci in range(Variadic.size(chars)):
        if v == comptime(chars[ci]):
            return comptime(True if not neg else False) 
    return comptime(False if not neg else True)

fn parse_toml(content: StringSlice) -> TomlType[content.origin]:
    var idx = 0

    var base = TomlType[content.origin].new_table()
    var data = iter(content.as_bytes())
